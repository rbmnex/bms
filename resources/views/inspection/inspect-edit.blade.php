...
<div class="row">
    <div class="col-12 mb-4">
        <div class="card border-light shadow-sm components-section">
            ...
            <div class="card-body">
                <form method="POST" id="inspectForm" action="{{route('inspect.submit')}}" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="inspector_name" class="form-control" id="inspectorNameTxt" value="{{$inspect->name}}" required>
                        </input>
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                ...
                                <input data-datepicker="" name="inspectionDate" class="form-control" id="inspectionDateTxt" value="{{$inspect->inspection_date}}" required>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        <div class="col">
                            <div class="form-group">
                                ...
                                <input type="number" name="nextYear" class="form-control" id="nextYearTxt" value="{{$inspect->next_year}}">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                ...
                                <input type="text" name="accident_record" class="form-control" id="accidentRecordTxt" value="{{$inspect->accident_record}}">
                                </input>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                        <div class="col">
                            <div class="form-group">
                                ...
                                <input type="text" name="flood_level" class="form-control" id="floodLevelTxt" value="{{$inspect->flood_level}}">
                                </input>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="mainRemark" id="mainRemarkTxt" rows="4" value="{{$inspect->remark}}"></textarea>
                    </div>
                    <hr>
...
<option value="5" title="Being heavily and critically damaged and possibly affecting the safety of traffic, it is necessary to implement emergency temporary repair work immediately or rehabilitation work without delay after the provision of a load limitation traffic sign">5</option>     
                        ...
                        <div class="col">
                            <textarea class="form-control mb-1" name="remark-{{$item->id}}" id="remarkTxt-{{$item->id}}" rows="3" placeholder="Remark Here...." value="{{$item->remark}}">{{$item->remark}}</textarea>
                            ...
                            <div class="form-file custom-file">
                                ...
                                <img class="pt-1" id="imgComp-{{$item->id}}" src="{{isset($photo->path) ? asset('storage/inspection/'.$photo->path) : ''}}">
                                <input type="hidden" name="photo_path-{{$item->id}}" id="photoHdn-{{$item->id}}" value="{{$photo->path}}">
                                @endforeach
                                @else
                                <img class="pt" id="imgComp-{{$item->id}}">
                    ...
                    <div class="row">
                        <div class="col">
                            <div class="form-group">
                                ...
                                <input type="text" name="name" class="form-control" id="nameTxt" value="" readonly required>
                                </input>
                                <div class="invalid-feedback"></div>
                            </div>
                        </div>
                    ...
                    <input type="hidden" name="inspect_id" id="inspect_id" value="{{$inspect->id}}">
...
@isset($members)
@foreach($members as $comp)
    $("#ratingSlt-{{$comp->id}}").val("{{$comp->condition_rating}}").attr("selected", "selected");
    $("#damageSlt-{{$comp->id}}").val("{{$comp->damage_id}}").attr("selected", "selected");
@endforeach
@endisset
    ...
    function saveInfo() {
        ...
        $("#inspectForm").attr("action","{{route('inspect.update')}}")
        $('#inspectForm').submit();
    }
@if($inspect->status == 'ONHOLD')    
    function deleteInfo() {
        ...
        $('#nameTxt').removeAttr("required");
        $('#emailTxt').removeAttr("required");
        $("#inspectForm").attr("action","{{route('delete.inspect')}}")
        $('#inspectForm').submit();
    }
@endif
    $(function() {
